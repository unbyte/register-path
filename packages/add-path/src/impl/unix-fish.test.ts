...
it.each([undefined, resolve('fixtures', 'config')])(
  ...
  async (directory) => {
    ...
    const path = join(directory ?? join(home, '.config'), 'fish', 'conf.d', 'add-path-example.fish')
    const beforeWrite = vi.fn(() => false)
    expect(await addFishPath('/opt/bin', { name: 'example', beforeWrite })).toEqual({
      path,
      status: 'skipped',
    })
...
it('rejects a relative XDG_CONFIG_HOME', async () => {
  ...
  await expect(addFishPath('/opt/bin', { name: 'example' })).rejects.toThrow('must be absolute')
  expect(readConfig).not.toHaveBeenCalled()
})

it('reads once when no approval callback is provided', async () => {
  expect((await addFishPath('/opt/bin', { name: 'example' })).status).toBe('added')

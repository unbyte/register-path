...
describe('addPosixPath', () => {
  ...
  it('preserves the cause and target of a write failure', async () => {
    ...
    await expect(addPosixPath(path, '/opt/bin', { name })).rejects.toMatchObject({
      path,
      cause: error,
    })
  })
  ...
  it('does not write if reading the latest content fails', async () => {
    ...
    await expect(
      addPosixPath(path, '/opt/bin', { name, beforeWrite: () => true }),
  ...
  it.each([
    ...
    await expect(addPosixPath(path, '/opt/bin', { name, beforeWrite })).resolves.toEqual({
      path,
      status: 'unchanged',
    })
  ...
  it.each(['\n', '\r\n'])(
    ...
    async (newline) => {
      ...
      await addPosixPath(path, '/opt/bin', { name })
  ...
  it('recognizes the escaped path when installed again', async () => {
    ...
    await addPosixPath(path, directory, { name })
    const content = vi.mocked(writeConfig).mock.calls[0][1] as string
    vi.mocked(readConfig).mockResolvedValue(content)
    const beforeWrite = vi.fn(() => true)
    await expect(addPosixPath(path, directory, { name, beforeWrite })).resolves.toEqual({
      path,
      status: 'unchanged',
    })
    expect(beforeWrite).not.toHaveBeenCalled()
  ...
  it.each(['# exampleXapp start', '# example.app.extra end'])(
    ...
    async (content) => {
      vi.mocked(readConfig).mockResolvedValue(content)
      await expect(addPosixPath(path, '/opt/bin', { name })).resolves.toEqual({
        path,
        status: 'added',
      })
    },

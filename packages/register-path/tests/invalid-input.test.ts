...
describe.skipIf(process.platform === 'win32')('invalid-input', () => {
  ...
  it('rejects unsupported shells and invalid input before any callback', async () => {
    ...
    await expect(registerPath('/opt/bin', { name: 'example', beforeWrite })).rejects.toThrow(
      'Unsupported',
    )
    for (const directory of ['', 'relative', '/has:separator', '/has\nnewline', '/has\0nul']) {
      await expect(registerPath(directory, { name: 'example', beforeWrite })).rejects.toThrow(
        TypeError,
      )
    }
    await expect(registerPath('/opt/bin', { name: '../bad', beforeWrite })).rejects.toThrow(
      TypeError,
    )
